...
local function DigDungAction(inst)
    local target = FindEntity(inst, SEE_DUNG_DIST, function(item) 
        return not inst:HasTag("hasdung") and item:HasTag("dungpile") 
    end)
    if target then
        local act = BufferedAction(inst, target, ACTIONS.DIGDUNG)
        act.validfn = function() 
            return not inst:HasTag("hasdung") and target:HasTag("dungpile") 
        end
        return act
    end
end
...
local function MountDungAction(inst)
    ...
    if target then
        ...
        act.validfn = function() 
            return not inst:HasTag("hasdung") and target:HasTag("dungball") 
        end
        return act
    end
end
